...
def upsert_conference(data: ConferenceIn = Body(...)):
    with get_db() as conn:
        ...
        if data.conferenceId:
            cursor.execute(
                "SELECT conferenceId FROM HannaIvanova.Conference WHERE conferenceId = ?",
                data.conferenceId
            )
            ...
            cursor.execute("""
                UPDATE HannaIvanova.Conference
        ...
        # CREATE
        cursor.execute("""
            INSERT INTO HannaIvanova.Conference (title, startDate, endDate, isActive, description)
...
def init_conference_table():
    with get_db() as conn:
        ...
        IF NOT EXISTS (
            ...
            WHERE TABLE_SCHEMA = 'HannaIvanova' AND TABLE_NAME = 'Conference'
        )
        CREATE TABLE HannaIvanova.Conference (
        ...
        conn.commit()
        return {"status": "OK", "message": "Schema HannaIvanova and table Conference created"}
